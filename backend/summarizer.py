...
重要提醒：这是{lang_instruction}内容，请完全用{lang_instruction}进行优化，重点解决句子被时间戳拆分导致的不连贯问题！"""
...
请特别注意修复因时间戳分割导致的句子不完整问题。"""
    ...
    async def summarize(self, transcript: str, target_language: str = "zh") -> str:
        """
        生成视频转录的摘要
...
要求：写成流畅的段落文章，整合相关观点，避免简单罗列，突出核心思想和逻辑脉络。"""
            ...
            summary_with_meta = f"""# {meta_labels['title']}
...
<p style="color: #888; font-style: italic; text-align: center; margin-top: 16px;"><em>{meta_labels['disclaimer']}</em></p>"""
    ...
    def _get_fallback_labels(self, lang_code: str) -> dict:
        ...
        labels = {
            "en": {
                ...
                "content_description": "The transcript contains complete video speech content. Since intelligent summary cannot be generated currently, we recommend:",
