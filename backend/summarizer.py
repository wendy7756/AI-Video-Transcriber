...
class Summarizer:
    ...
    async def optimize_transcript(self, raw_transcript: str, target_language: str = "zh-tw") -> str:
        """
        最佳化轉錄文本：修正錯別字，按語意分段
        支援長文本分塊處理
        ...
        try:
            ...
            if len(preprocessed) > max_chars_per_chunk:
                ...
                return await self._format_long_transcript_in_chunks(preprocessed, detected_lang_code, max_chars_per_chunk, target_language)
            else:
                return await self._format_single_chunk(preprocessed, detected_lang_code, target_language)
...
輸出清理後的文本，保持原文結構。"""
    ...
    async def _format_single_chunk(self, chunk_text: str, transcript_language: str = 'zh-tw', target_language: str = 'zh-tw') -> str:
    ...
    async def _format_long_transcript_in_chunks(self, raw_transcript: str, transcript_language: str, max_chars_per_chunk: int, target_language: str = 'zh-tw') -> str:
        ...
        for i, c in enumerate(final_chunks):
            ...
            if i > 0:
                ...
                marker = f"[上文續：{prev_tail}]" if transcript_language == 'zh-tw' else f"[Context continued: {prev_tail}]"
                chunk_with_context = marker + "\n\n" + c
            try:
                oc = await self._format_single_chunk(chunk_with_context, transcript_language, target_language)
                # 移除上下文標記
                oc = re.sub(
